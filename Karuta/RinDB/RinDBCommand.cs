...
namespace LuminousVector.Karuta.RinDB
{
	...
	class RinDBCommand : Command
	{
		...
		void AddTag()
		{
			...
			RinDB.CreateTag(new TagModel("Fate/Stay Night: Unlimted Blade Works", "work", "Fate/Stay Night: UBW from the Fate series by TYPE-MOON") { parentID = fate.id });
		...
		void Rebuild()
		{
			...
			foreach (string f in dir)
			{
				...
				cmd.CommandText = $"INSERT INTO images (id, fileuri, timeadded, name, isnsfw) VALUES('{id}', '{Uri.EscapeDataString(file)}', '{epoch}', '{name}', '{file.Contains("NSFW")}'); INSERT INTO tagmap VALUES('{tag.id}{id}','{id}', '{tag.id}');";
