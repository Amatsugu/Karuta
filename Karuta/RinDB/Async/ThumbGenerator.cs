...
namespace LuminousVector.Karuta.RinDB.Async
{
	public static class ThumbGenerator
	{
		...
		public static void QueueThumb(ImageModel image)
		{
			_genQueue.Enqueue(image);
		}

		public static void QueueThumb(IEnumerable<ImageModel> images)
		{
		...
		{
			Karuta.StartTimer("RinDB.ThumbGenerator", e =>
			{
				if (_isGenerating)
					return;
				_isGenerating = true;
		...
		public static void GenerateThumb(ImageModel img)
		{
			if (img == null)
				return;
			...
			try
			{
				using (Image image = Image.FromFile(img.fileUri, true))
				{
					...
					using (var destImage = new Bitmap(width, height))
					{
						...
						destImage.Save(thumbUri, (ext == ".gif") ? ImageFormat.Gif : ImageFormat.Png);
						_genCount++;
					}
				}
			}catch(Exception e)
			{
				Karuta.Write($"{e.Message} {img.fileUri}");
				_failed.Add(img);
				return;
			}
		}
